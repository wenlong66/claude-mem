...
Durable backup index for raw tool I/O (schema v51). Written from the same ingest
choke point that feeds the observation generator, so it captures the same calls
without adding a second capture path.
...
**Why it exists**: `pending_messages` is the generation *queue* — rows are
